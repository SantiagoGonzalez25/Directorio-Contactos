...
**a) Equivalencias web vs React Native:**
*   `<div>` = `<View>`
*   `<p>` o `<span>` = `<Text>`
*   `<img>` = `<Image>`
*   `<input>` = `<TextInput>`
...
*   El sistema usa Flexbox por defecto. El valor predeterminado es `flexDirection: 'column'` (en web es 'row'). Esta decisión tiene todo el sentido en móviles, ya que las pantallas son verticales y el contenido fluye naturalmente de arriba hacia abajo.
...
*   **Stack:** Organiza las pantallas como una pila donde la nueva se coloca sobre la anterior. *Caso de uso:* Navegar desde una lista general hacia la pantalla de detalle de un contacto específico y poder regresar.

*   **Tabs:** Muestra pantallas principales accesibles mediante pestañas permanentes (generalmente abajo). *Caso de uso:* Una red social con secciones principales como Inicio, Buscar, Notificaciones y Perfil.

*   **Drawer:** Despliega un menú lateral oculto para agrupar opciones. *Caso de uso:* Una aplicación de banco o correo electrónico con acceso lateral a Configuración, Ayuda, Historial y Cerrar sesión.
...
*   **Elección para la Parte 2:** Para la aplicación del Directorio de Contactos, elegiría **Cloud Firestore**. El taller requiere manejar listas de usuarios con campos específicos (nombre, teléfono, ciudad), lo cual encaja perfectamente en el modelo de documentos. Además, la rúbrica exige específicamente crear una "colección" llamada `contactos`, una estructura nativa de Firestore.